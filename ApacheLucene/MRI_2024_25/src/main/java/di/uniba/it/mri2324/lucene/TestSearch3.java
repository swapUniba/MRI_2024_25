...
public class TestSearch3 {
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        qb.add(new TermQuery(new Term("chapter_text", "king")), BooleanClause.Occur.SHOULD);
        qb.add(new TermQuery(new Term("chapter_text", "cheshire")), BooleanClause.Occur.SHOULD);
        Query q = qb.build();
		...
		for (int i = 0; i < hits.length; i++) {
			...
			System.out.println(hitDoc.get("chapter") + ") " + hitDoc.get("chapter_text") + " " +  hits[i].score);
		}
		...
		Query query = new QueryParser("chapter_text", analyzer).parse("chapter_text: roam~0.7");
